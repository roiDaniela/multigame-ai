...
class TicTacToe extends React.Component {
  ...
  render() {
    ...
    if (this.state.winner) {
      ...
      announcement = (
        <div className="tic-announcement">
          <p>{ msg }</p>
          <button className={"tic-button"} onClick={ this.reset.bind(this) }>Reset</button>
        </div>
      );
    }
    ...
    const grid = board.map((row, rowInd) => {
      const cells = row.map((cell, cellInd) => {
        const classString = cell > 0 ? cell === 1 ? 'tic-cell-p1' : 'tic-cell-p2' : 'tic-cell';
...
TicTacToe.propTypes = { width: PropTypes.number };
